...
All notable changes to `@absolutejs/deploy`.
...
- **Trusted native certification accepts a bounded portable Sigstore bundle and full workflow identity before retaining provenance** (`NativeReleaseCertificationVerification`, `NativeReleaseRegistry`, `NativeReleaseRegistryOptions`)

## 0.26.0 — 2026-09-17

### Added

- **Native release registries retain content-addressed acceptance certification and optional trusted provenance with channel promotion** (`AndroidNativeReleaseMetadata`, `IosNativeReleaseMetadata`, `NativeReleaseCertification`, `NativeReleaseCertificationProvenance`, `NativeReleaseCertificationReceipt`, `NativeReleaseCertificationRequirement`, `NativeReleaseChannel`, `NativeReleasePublication`, `NativeReleaseRegistry`, `NativeReleaseRegistryOptions`)

## 0.25.14 — 2026-09-13

### Fixed

- **Expo staged-rollout pause, resume, cancellation, and fleet fallback transitions receive stable activation identities so installed apps can select the trusted server state.** (`createMobileUpdateHandler`, `createMobileUpdateRegistry`)
